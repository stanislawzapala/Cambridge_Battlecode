...
import heapq
# 4. For efficient ring buffer of recent facts
import collections
...
class Player:
    def __init__(self):
        ...
        self.is_bot_temporary_blocked: bool = False
    ...
    def calculate_astar_path(self, ct: Controller, start: Position, target: Position, w: int, h: int, bot_id: int, my_team: Team, blocked_directions = []) -> list[Direction] | None:
        """
            ...
            for d in DIRECTIONS_PREFERENCE:
                ...
                if next_pos not in cost_so_far or new_cost < cost_so_far[next_pos]:
                    ...
                    if ct.is_in_vision(next_pos):
                        ...
                        print("d")
                        print(d)
                        print("blocked directions")
                        print(blocked_directions)
                        if d in blocked_directions:
                            is_blocked = True
    ...
    def run(self, ct: Controller) -> None:
        ...
        # ==========================================
            ...
            if my_id not in self.bot_targets:
                ...
                # previous position
                self.previous_position = [ct.get_position(my_id)]

            cur_position = ct.get_position(my_id)
            self.previous_position.append(cur_position)
            self.previous_position = self.previous_position[-2:]

            if cur_position != self.previous_position[-2] and self.is_bot_temporary_blocked:
                self.is_bot_temporary_blocked = False
                delta_x = cur_position[0] - self.previous_position[-2][0]
                delta_y = cur_position[1] - self.previous_position[-2][1]

                if delta_x == 1 and delta_y == 1:
                    self.blocked_directions = [Direction.NORTHWEST]
                elif delta_x == 1 and delta_y == 0:
                    self.blocked_directions = [Direction.WEST]
                elif delta_x == 1 and delta_y == -1:
                    self.blocked_directions = [Direction.SOUTHWEST]
                elif delta_x == 0 and delta_y == 1:
                    self.blocked_directions = [Direction.NORTH]
                elif delta_x == 0 and delta_y == -1:
                    self.blocked_directions = [Direction.SOUTH]
                elif delta_x == -1 and delta_y == 1:
                    self.blocked_directions = [Direction.NORTHEAST]
                elif delta_x == -1 and delta_y == 0:
            ...
            for _ in range(5): 
                ...
                print("target_pos")
                print(target_pos)
                print("not self body paths")
                print(not self.bot_paths[my_id])
                ...
                if target_pos and not self.bot_paths[my_id]: 
                    ...
                    print(b_id is not None)
                    print(not can_walk_on_enemy)
                    
                    out_of_bounds = not (0 <= greedy_pos.x < map_width and 0 <= greedy_pos.y < map_height)
                    
                    if not is_hard_obstacle and not out_of_bounds and not self.is_bot_temporary_blocked:
                        # Droga wolna! Zapisujemy jeden zachłanny krok.
                        self.bot_paths[my_id] = [greedy_dir]
                    else:
                        # KROK 2: Uderzenie w przeszkodę -> Tryb Awaryjny (A*)
                        print("emergency mode 429")
                        self.bot_paths[my_id] = self.calculate_astar_path(ct, my_pos, target_pos, map_width, map_height, my_id, my_team, self.blocked_directions)
                ...
                print("Before FAZA WYKONANIA")
                print(self.bot_paths[my_id])
                if self.bot_paths[my_id]:
                    ...
                    # PRZYPADEK C: Niespodziewana przeszkoda NA TRASIE z A*
                        ...
                        if env in [Environment.WALL, Environment.ORE_TITANIUM, Environment.ORE_AXIONITE] or b_id is not None:
                            ...
                            self.bot_paths[my_id] = []
                            self.is_bot_temporary_blocked = True
                            print("next blocked direction")
                            print(next_dir)
                            print(self.blocked_directions)
                            self.blocked_directions.append(next_dir)
                            print("penultimate 476")
